"""
V2 Showcase - Demonstrating Lessons Learned
5-7 slides showing professional-grade presentation generation
"""
...
class ShowcaseV2:
    """Showcase demonstrating professional presentation generation"""
    ...
    def create_table_comparison(self, slide, left_header, right_header, rows):
        ...
        for i, (left, right) in enumerate(rows, 1):
            ...
            left_cell.margin_left = Inches(0.2)
            left_cell.margin_right = Inches(0.2)
            ...
            right_cell.margin_left = Inches(0.2)
            right_cell.margin_right = Inches(0.2)

    def create_question_slide(self, title, subtext=None):
        """Create a provocative question slide"""
        ...
        question_box = slide.shapes.add_textbox(
            Inches(1), Inches(2), Inches(14), Inches(3)
        )
        ...
        if subtext:
            subtext_box = slide.shapes.add_textbox(
                Inches(2), Inches(5.5), Inches(12), Inches(2)
            )
    ...
    def generate_advanced_chart(self):
        """Generate chart with advanced QuickChart features"""
        ...
        chart_path = self.output_dir / "showcase_chart_advanced.png"
        if APIUtils.generate_quickchart(chart_config, str(chart_path), width=1400, height=700):
            print(f"  [OK] Advanced chart saved")
            return chart_path
        return None

    def build_showcase(self):
        """Build the V2 showcase presentation"""
        print("="*60)
        print("BUILDING V2 SHOWCASE")
        print("Demonstrating lessons learned")
        print("="*60)
        ...
        title_box = slide1.shapes.add_textbox(
            Inches(0.5), Inches(2.5), Inches(7), Inches(2)
        )
        ...
        subtitle_box = slide1.shapes.add_textbox(
            Inches(0.5), Inches(5), Inches(7), Inches(0.8)
        )
        ...
        print("[5/7] Advanced chart with data labels & annotations")
        ...
        if chart_path:
            ...
            insight_frame.text = "3.7 month payback period  •  221 patterns encoded  •  Permanent institutional asset"

            insight_para = insight_frame.paragraphs[0]
            insight_para.font.name = 'Inter'
            insight_para.font.size = Pt(20)
        ...
        statement_box = slide6.shapes.add_textbox(
            Inches(1.5), Inches(3), Inches(13), Inches(3)
        )
        ...
        subtext_box = slide6.shapes.add_textbox(
            Inches(2), Inches(6.5), Inches(12), Inches(1)
        )
        subtext_frame = subtext_box.text_frame
        ...
        # Save presentation
        output_file = self.output_dir / "V2_Showcase_Lessons_Learned.pptx"
        self.prs.save(str(output_file))

        print("\n" + "="*60)
        print("V2 SHOWCASE COMPLETE")
        print("="*60)
        print(f"Saved to: {output_file}")
        print(f"Total slides: {len(self.prs.slides)}")
        print("\nImprovements demonstrated:")
        print("  [OK] Professional title illustration")
        print("  [OK] Team photos with bios")
        print("  [OK] Provocative questions")
        print("  [OK] Comparison table (structured)")
        print("  [OK] Advanced chart (data labels + annotations)")
        print("  [OK] Short statements (not paragraphs)")
